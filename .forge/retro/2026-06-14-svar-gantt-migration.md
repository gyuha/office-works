...
- Do differently next time:
  ...
  - **SVAR Gantt는 막대 색을 type 스코프 CSS 변수(`--wx-gantt-task-*`, `.wx-task`에 스코프)로 칠한다 — `type`을 부서값으로 바꾸면 fill이 깨진다.** 막대 div의 `data-task-id`를 이용해 `.wx-bar[data-task-id="…"]{--wx-gantt-task-*}`를 `<style>`로 주입하면 type 오염 없이 per-task 색상 + 네이티브 진척 오버레이 유지. (ADR-0008 "CSS 변수 경로" 범위 내, 코드 주석에 명시됨.)
...
- ADR added: none (부서색 CSS 변수 경로는 기존 ADR-0008 범위 내; 테스트 러너 부재는 사용자 판단으로 회고 로그에만 기록)
